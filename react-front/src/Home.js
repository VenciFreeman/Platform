    ...
    render()
    {
        return(
                <Router>
                    <div>       
                            <Paper>
                                <Link to="/Home"><Button>主页</Button></Link>
                                <Link to="/catalog"><Button>书籍浏览</Button></Link>
                                <Link to="/mycart"><Button>购物车</Button></Link>
                                <Link to="/order"><Button>订单管理</Button></Link>
                                <Button onClick={this.Logout}>注销</Button>                                                                
                            </Paper>  
                        <Content>
                            <Route path="/Home" render={() => <Carousel autoplay class="slick-slide">
                            <h1>welcom to bookstore!</h1>
                            </Carousel>} />  
                            <Route path="/catalog" render={() => <Catalog showDetail={this.jumpToDetail} />} />
                            <Route path="/mycart" render={() => <ShoppingList uID={this.state.currentUserId} />}  />
                            <Route path="/bookpage" render={() => <BookPage book={this.state.bookToDisplay} />} />
                            <Route path="/order" render={() => <OrderPage showDetail={this.showOrderItems}  judge={this.state.currentUserId}/>}/>
